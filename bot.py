...
from markups import main_menu_markup, admin_menu_markup
...
async def set_user_city_start(message: types.Message):
    ...
    btn1 = types.KeyboardButton('Меню')
    markup.add(btn1)
    text = 'Введите название своего города'
    await message.answer(text, reply_markup=markup)
...
async def user_city_chosen(message: types.Message, state: FSMContext):
    ...
    city = await state.get_data()
    text = f'Установлен свой город {city.get("waiting_user_city")}'
    await message.answer(text, reply_markup=main_menu_markup)
    await state.finish()
...
async def get_user_city_weather(message):
    ...
    if city is None:
        await message.answer('У вас не установлен свой город, нажмите кнопку "Установить свой город"')
        return
...
async def callback_query(call, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if query_type == 'report':
            ...
            report = orm.get_report(int(report_id))
            inline_markup = types.InlineKeyboardMarkup()
            inline_markup.add(
...
@dp.message_handler(
    lambda message: message.from_user.id in bot_config.tg_bot_admin and message.text == 'Администратор')
async def admin_panel(message: types.Message):
    text = f'Админ-панель'
    await message.answer(text, reply_markup=admin_menu_markup)
